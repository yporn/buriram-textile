...
export function SiteFooter() {
  return (
    <footer className="bg-earth-deep text-cream/90">
      {/* เส้นทองบาง ๆ คู่กับแถบครามที่หัวเว็บ ให้เป็น bookend เดียวกัน */}
      ...
      <div className="mx-auto max-w-6xl px-6 py-10 flex flex-col sm:flex-row sm:items-start justify-between gap-8">
        <div>
          <p className="font-heading text-cream text-lg mb-1.5">ผ้าทอบุรีรัมย์</p>
        ...
        <nav className="flex flex-col gap-2 text-sm">
          {FOOTER_LINKS.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className="opacity-80 hover:opacity-100 hover:text-gold transition w-fit"
            >
              {link.label}
            </Link>
          ))}
